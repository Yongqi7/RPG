...
from projects.mmdet3d_plugin.datasets.fusion_utils import visualize_bev, transform_coordinates
...
from projects.configs.custom_fusion_registry import get_fusion_model
...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        if self.enable_fusion:
            if fusion_cfg is not None:
                ...
                print(f"Start fusing BEV features, Using fusion model: {fusion_model_name}")
    ...
    def _forward_single_frame_train(
        ...
        if self.enable_fusion:
            ...
            self.eval()
            with torch.no_grad():
                sender_bev_embed, _ = self.get_bevs(
        ...
        det_output = self.pts_bbox_head.get_detections(
            fusion_bev_embed if self.enable_fusion else bev_embed,
    ...
    def forward_track_train(self,
        ...
        for i in range(num_frame):
            ...
            # obtain the sener track_instances
            ...
            if self.enable_track_fusion and (sender_img_single is not None):
                ...
                track_instances = self.cross_agent_query_interaction(sender_track_instances,
                                                                    track_instances,
                                                                    ego2sender_rt,
                                                                    pc_range
                                                                )
                ...
                # bev_embed,bev_pos = self._get_coop_bev_embed(fusion_bev_embed, bev_pos, track_instances, track_nums_new-add_nums)
    ...
    def _forward_single_frame_inference(
        ...
        # TODO: remove the visualization of BEV after debugging
        # if self.debug:
        #     if self.enable_fusion:
        #         visualize_bev(bev_embed, backup_sender_bev, sender_bev_embed, fusion_bev_embed, None, None, sender_img_metas, vis_idx=None, debug=self.debug)
