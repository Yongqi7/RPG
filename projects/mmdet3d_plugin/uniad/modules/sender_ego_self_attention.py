...
class SenderEgoSelfAttention(BaseModule):
    """An attention module used for fusion of sender and ego features.

    Args:
        embed_dims (int): The embedding dimension of Attention.
            Default: 256.
        num_heads (int): Parallel attention heads. Default: 8.
        num_levels (int): The number of feature maps used in Attention. Default: 4.
    ...
    def __init__(self,
        ...
        self.sampling_offsets = nn.Linear(
            embed_dims * 2, num_heads * 2 * num_levels * num_points * 2)
        self.attention_weights = nn.Linear(
            embed_dims * 2, num_heads * 2 * num_levels * num_points)
    ...
    def init_weights(self):
        ...
        grid_init = (grid_init /
            ...
            self.num_heads, 1, 1,
            2).repeat(1, self.num_levels * 2, self.num_points, 1)  # For sender and ego
    ...
    def forward(self,
                ...
                flag='decoder',
                **kwargs):
        """Forward Function of Sender-Ego Self-Attention.
        ...
        if value is None:
            ...
            value = torch.stack([query, query], 1).reshape(bs * 2, len_bev, c)  # Sender and ego
        ...
        # Concatenate sender and ego features
        ...
        value = value.reshape(bs * 2, num_value, self.num_heads, -1)  # Sender and ego

        sampling_offsets = self.sampling_offsets(query)
        sampling_offsets = sampling_offsets.view(
            bs, num_query, self.num_heads, 2, self.num_levels, self.num_points, 2)  # 2 for sender and ego
        attention_weights = self.attention_weights(query).view(
            bs, num_query, self.num_heads, 2, self.num_levels * self.num_points)
        ...
        attention_weights = attention_weights.permute(0, 3, 1, 2, 4, 5)\
            .reshape(bs * 2, num_query, self.num_heads, self.num_levels, self.num_points).contiguous()
        sampling_offsets = sampling_offsets.permute(0, 3, 1, 2, 4, 5, 6)\
            .reshape(bs * 2, num_query, self.num_heads, self.num_levels, self.num_points, 2)
        ...
        # fuse sender value and current value
        # (num_query, embed_dims, bs*num_bev_queue)-> (num_query, embed_dims, bs, 2)
        output = output.view(num_query, embed_dims, bs, 2)
