...
class AgentQueryFusion(nn.Module):
    ...
    def _dis_filt(self, veh_pts, inf_pts, veh_dims):
        """
        filter suitable inf according to distance
        distance is too small, then ignore this inf
        """
    ...
    def forward(self, inf, veh, ego2other_rt, other_agent_pc_range, threshold=0.3):
        ...
        # inf_ref_pts inf2veh
        veh_ref_pts = np.linalg.inv(ego2other_rt[0].cpu().numpy().T)
        ...
        remove_ego_ins = True
        if remove_ego_ins:
            # TODO: Adjust the vehicle parameters.
        ...
        inf.query[..., :self.embed_dims] = self.cross_agent_align_pos(torch.cat([inf.query[..., :self.embed_dims],inf2veh_r], -1)) #[N, 265]
