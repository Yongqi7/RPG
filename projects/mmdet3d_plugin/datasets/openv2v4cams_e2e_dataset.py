...
class OpenV2V4CAMSE2EDataset(NuScenesDataset):
    r"""OpenV2V 4 cams E2E Dataset.
    ...
    def __init__(self,
        ...
        self.enable_map_fusion = enable_map_fusion

        # cache the sender vehicle information for faster access 
        self.sender_cache = {}
        ...
        self.nusc_maps = {
            'Town01': NuScenesMap(dataroot=self.data_root, map_name='Town01'),
            'Town02': NuScenesMap(dataroot=self.data_root, map_name='Town02'),
            'Town03': NuScenesMap(dataroot=self.data_root, map_name='Town03'),
            'Town04': NuScenesMap(dataroot=self.data_root, map_name='Town04'),
            'Town05': NuScenesMap(dataroot=self.data_root, map_name='Town05'),
            'Town06': NuScenesMap(dataroot=self.data_root, map_name='Town06'),
            'Town07': NuScenesMap(dataroot=self.data_root, map_name='Town07'),
            'Town10HD': NuScenesMap(dataroot=self.data_root, map_name='Town10HD'),
        }
    ...
    def load_annotations(self, ann_file):
        ...
        Returns:
            list[dict]: List of annotations sorted by scene_token and timestamps.
        """
        if self.file_client_args['backend'] == 'disk':
            data = pickle.loads(self.file_client.get(ann_file))

            # fisrt sort by scene_token, then by timestamp to aviod mixed scenes
            data_infos = list(
                sorted(data['infos'], key=lambda e: (e['scene_token'], e['timestamp']))
            )
    ...
    def prepare_train_data(self, index):
        ...
        Returns:
            dict: Training data dict of the corresponding index.
                img: queue_length, num_cameras, 3, H, W
        ...
        data_queue_out = self.union2one(data_queue, is_sender=False)

        # Prepare sender data if enable fusion
        if self.enable_fusion or self.enable_track_fusion or self.enable_map_fusion:
            print('----start preparing sender data----')
            sender_data_queue = []
            queue_length = self.queue_length
            for idx in range(queue_length):
                sender_data = self.prepare_sender_data(index - idx)
                if sender_data is not None:
                    sender_data_queue.insert(0, copy.deepcopy(sender_data))
                else:
                    return None
            sender_data_queue_out = self.union2one(sender_data_queue, is_sender=True)
            
            # Use copy for data transfer
            data_queue_out['sender_img'] = sender_data_queue_out['img'].copy() if isinstance(sender_data_queue_out['img'], np.ndarray) else sender_data_queue_out['img']
            data_queue_out['sender_img_metas'] = copy.deepcopy(sender_data_queue_out['img_metas'])
            data_queue_out['sender_gt_bboxes_3d'] = copy.deepcopy(sender_data_queue_out['gt_bboxes_3d'])
            data_queue_out['sender_gt_labels_3d'] = copy.deepcopy(sender_data_queue_out['gt_labels_3d'])
            data_queue_out['sender_gt_inds'] = copy.deepcopy(sender_data_queue_out['gt_inds'])
            data_queue_out['sender_gt_past_traj'] = copy.deepcopy(sender_data_queue_out['gt_past_traj'])
            data_queue_out['sender_gt_past_traj_mask'] = copy.deepcopy(sender_data_queue_out['gt_past_traj_mask'])
        return data_queue_out

    def build_sender_cache(self):
        """Build cache of sender vehicle information"""
        for idx, info in enumerate(self.data_infos):
            match = re.match(r'.*/(\d{4}_\d{2}_\d{2}_\d{2}_\d{2}_\d{2})_(\d+)/\d+/(\d+)', info['token'])
            if not match:
                continue
            
            scene, vehicle_id, frame_id = match.groups()
            translation = np.array(info['ego2global_translation'][:2])
            rotation = info['ego2global_rotation']
            
            if scene not in self.sender_cache:
                self.sender_cache[scene] = {}
            if frame_id not in self.sender_cache[scene]:
                self.sender_cache[scene][frame_id] = []
                
            self.sender_cache[scene][frame_id].append((idx, vehicle_id, translation, rotation))

    def prepare_sender_data(self, i):
        """
        Prepare sender data using cached information.
        Args:
            i (int): Index for accessing the target sender data.
        Returns:
            dict: Training data dict with processed image and metadata.
        """
        if not hasattr(self, 'sender_cache') or not self.sender_cache:
            self.build_sender_cache()
        ...
        try:
            match = re.match(r'.*/(\d{4}_\d{2}_\d{2}_\d{2}_\d{2}_\d{2})_(\d+)/\d+/(\d+)', input_dict['sample_idx'])
            if not match:
                return None
                
            target_scene, target_vehicle_id, target_frame_id = match.groups()
            target_translation = np.array(input_dict['ego2global_translation'][:2])
            target_rotation = Quaternion(input_dict['ego2global_rotation'])
            target_can_bus = copy.deepcopy(input_dict['can_bus'])
            target_l2g_r_mat = copy.deepcopy(input_dict['l2g_r_mat'])
            target_l2g_t = copy.deepcopy(input_dict['l2g_t'])

            rotation_diffs = []
            for idx, vehicle_id, translation, rotation in self.sender_cache[target_scene][target_frame_id]:
                if vehicle_id == target_vehicle_id:
                    continue
                    
                distance = np.linalg.norm(translation - target_translation)
                if not (0 <= distance <= 51.2):
                    continue
                    
                relative_rotation = Quaternion(rotation).inverse * target_rotation
                rotation_diffs.append((idx, relative_rotation.angle))

            suitable_sender = max(rotation_diffs, key=lambda x: x[1])[0] if rotation_diffs else i

            input_dict = self.get_data_info(suitable_sender)
            if input_dict is None:
                return None
            ...
            if not self._is_valid_example(example):
                return None

            # self._update_transformation(example, target_l2g_r_mat, target_l2g_t)
            self._update_transformation(example, target_can_bus)
            return example
            
        except (KeyError, IndexError, ValueError) as e:
            return None
        
    def prepare_test_sender_data(self, i):
        """
        Prepare sender data using cached information.
        Args:
            i (int): Index for accessing the target sender data.
        ...
        The function will always return a valid example. If no suitable sender is found,
        it returns the ego vehicle's data itself.
        """
        if not hasattr(self, 'sender_cache') or not self.sender_cache:
            self.build_sender_cache()

        # Get ego data first as fallback
        ego_dict = self.get_data_info(i)
        if ego_dict is None:
            return None  # This should never happen as i should always be valid
            
        # Prepare ego data as fallback
        self.pre_pipeline(ego_dict)
        ego_example = self.pipeline(ego_dict)

        try:
            match = re.match(r'.*/(\d{4}_\d{2}_\d{2}_\d{2}_\d{2}_\d{2})_(\d+)/\d+/(\d+)', ego_dict['sample_idx'])
            if not match:
                return ego_example
                
            target_scene, target_vehicle_id, target_frame_id = match.groups()
            target_translation = np.array(ego_dict['ego2global_translation'][:2])
            target_rotation = Quaternion(ego_dict['ego2global_rotation'])
            target_can_bus = copy.deepcopy(ego_dict['can_bus'])
            target_l2g_r_mat = copy.deepcopy(ego_dict['l2g_r_mat'])
            target_l2g_t = copy.deepcopy(ego_dict['l2g_t'])
            ...
            # If no suitable sender found, return ego_example
            if not rotation_diffs:
                return ego_example

            suitable_sender = max(rotation_diffs, key=lambda x: x[1])[0]
            sender_dict = self.get_data_info(suitable_sender)
            if sender_dict is None:
                return ego_example

            self.pre_pipeline(sender_dict)
            sender_example = self.pipeline(sender_dict)

            if not self._is_valid_example(sender_example):
                return ego_example

            # Update transformation
            self._update_transformation(sender_example, target_can_bus)
            return sender_example
            
        except (KeyError, IndexError, ValueError) as e:
            return ego_example

    def _is_valid_example(self, example):
        """Validate processed example data"""
        return (example is not None and
                not (self.filter_empty_gt and not (example['gt_labels_3d']._data != -1).any()) and
                example['gt_labels_3d'].data.shape[0] == example['gt_fut_traj'].shape[0] == example['gt_past_traj'].shape[0])

    def _update_transformation(self, example, target_can_bus):
        """
        Update transformation parameters safely
        
        Args:
            example: sender data containing img_metas
            target_can_bus: ego vehicle data
            
        Raises:
            ValueError: If input arrays have incorrect lengths
        """
        target_can_bus = copy.deepcopy(target_can_bus)
        can_bus_data = copy.deepcopy(example['img_metas'].data['can_bus'])
        
        if len(target_can_bus) < 3 or len(can_bus_data) < 6:
            raise ValueError("Input arrays must have sufficient length")
            
        sender_pos = example['img_metas'].data['can_bus'][:2]
        sender_angle = example['img_metas'].data['can_bus'][-1]
        
        ego_pos = target_can_bus[:2]
        ego_angle = target_can_bus[-1]
        
        can_bus_data[:2] = ego_pos
        can_bus_data[2] = ego_angle
        can_bus_data[3:5] = sender_pos  
        can_bus_data[5] = sender_angle  
        
        example['img_metas'].data['can_bus'] = can_bus_data
    ...
    def prepare_test_data(self, index):
        ...
        Returns:
            dict: Training data dict of the corresponding index.
                ...
                gt_inds: gt_inds of each frame(list)
        """
        print('======prepare_sender_test_data====')
        input_dict = self.get_data_info(index)
        self.pre_pipeline(input_dict)
        example = self.pipeline(input_dict)
        data_dict = {}
        ...
        # Prepare sender data if fusion is enabled
        if self.enable_fusion or self.enable_track_fusion or self.enable_map_fusion:
            sender_data_queue = []
            sender_data = self.prepare_test_sender_data(index)
            if sender_data is not None:
                sender_data_queue.insert(0, copy.deepcopy(sender_data))

            # Combine sender data into a single dictionary
            if sender_data_queue:
                sender_data_queue_out = self.sender_test_union2one(sender_data_queue)

                # Integrate sender data into the main data dictionary
                data_dict['sender_img'] = sender_data_queue_out.get('img', None)
                data_dict['sender_img_metas'] = sender_data_queue_out.get('img_metas', None)
            else:
                # Add default values when sender_data_queue is empty
                data_dict['sender_img'] = None
                data_dict['sender_img_metas'] = None
        return data_dict

    def union2one(self, queue, is_sender=False):
        """
        ...
        for i, each in enumerate(queue):
            metas_map[i] = copy.deepcopy(each['img_metas'].data)
            if not is_sender:
                if i == 0:
    ...
    def sender_test_union2one(self, queue):
        """
        Convert sample dict into one single sample with only selected fields.
        """
        # Collect required data from the last element in the queue
        imgs_list = [each['img'][0].data for each in queue]
        
        # Prepare img and img_metas
        queue[-1]['img'] = DC(torch.stack(imgs_list), cpu_only=False, stack=True)
        
        # Prepare img_metas
        metas_map = {i: copy.deepcopy(each['img_metas'][0].data) for i, each in enumerate(queue)}
        queue[-1]['img_metas'] = DC(metas_map, cpu_only=True)
        
        # Return the last element with required fields only
        return queue[-1]
    ...
    def _evaluate_single(self,
        ...
        if 'track' in self.eval_mod:
            cfg = config_factory("tracking_cooper_uniad_2024")
