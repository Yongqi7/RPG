_base_ = ["../_base_/datasets/openv2v-3d.py",
...
img_norm_cfg = dict(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True) #this is true
#img_norm_cfg = dict(mean=[103.530, 116.280, 123.675], std=[1.0, 1.0, 1.0], to_rgb=False) #this is original uniad config
# For openv2v dataset we usually do 1-class detection
class_names = [
    "car",
]
...
enable_ref_pts_fusion = True
debug = False  # Set to True to enable debug mode, which will print additional information during training and testing.
...
model = dict(
    ...
    img_backbone=dict(
        ...
        num_stages=4,
        out_indices=(1, 2, 3),
        #freeze: frozen_stages=4, requires_grad=false
        #train: frozen_stages=1, requires_grad=true
        frozen_stages=4,
    ...
    enable_track_fusion=enable_track_fusion,
    enable_ref_pts_fusion=enable_ref_pts_fusion,
    use_oracle_ref_pts=use_oracle_ref_pts,
    #offline_results_path=offline_results_path,
    #is_ego_agent=is_ego_agent,
    fusion_cfg=fusion_cfg,
    #track_fusion_cfg=track_fusion_cfg,
    #Freeze: freeze_img_backbone=True, freeze_img_neck=True
...
img_root = "/home/UNT/yz0364/M3CAD"
...
ann_file_test=info_root + f"m3cad_carla_ue5_infos_temporal_val.pkl"
        ...
        transforms=[
            ...
            dict(
                type="CustomCollect3D", keys=[
                                            "gt_bboxes_3d", # only for visualization if DEBUG
                                            "img",
                                            "timestamp",
                                            "l2g_r_mat",
                                            "l2g_t",
