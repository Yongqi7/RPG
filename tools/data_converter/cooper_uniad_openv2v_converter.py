...
nus_attributes = ('vehicle.moving', 'vehicle.parked', 'vehicle.stopped', 'None')
...
def create_nuscenes_infos(root_path,
    ...
    if version == 'v1.0-trainval':
        ...
        elif info_prefix.startswith('openv2v'):
            train_scenes = splits.mini_train
            val_scenes = splits.mini_val
        elif info_prefix.startswith('m3cad_carla'):
            train_scenes = splits.mini_train
            val_scenes = splits.mini_val
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, None, train_scenes, val_scenes, test, max_sweeps=max_sweeps)
...
def obtain_sensor2top(nusc,
                      ...
                      e2g_r_mat,
                      sensor_type='lidar'):
    """Obtain the info with RT matrix directly from sensor to LiDAR.
    ...
    R = l2e_r_s_mat.T @ np.linalg.inv(l2e_r_mat).T
    T = (l2e_t_s @ np.linalg.inv(l2e_r_mat).T) - l2e_t @ np.linalg.inv(l2e_r_mat).T
...
def export_2d_annotation(root_path, info_path, version, mono3d=True):
    ...
    nusc_infos = mmcv.fileio.io.load(info_path)['infos']
    nusc = NuScenes(version=version, dataroot=root_path, verbose=True)
    # info_2d_list = []
    cat2Ids = [
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            ...
            coco_2d_dict['images'].append(
                dict(
                    file_name=cam_info['data_path'].split('data/openv2v_4_cams')
                    [-1],
