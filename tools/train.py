...
def main():
    ...
    if 'model' in cfg:
        ...
        print("Available keys:", list(cfg.keys()))
        raise KeyError("Configuration file must contain 'model' section")
    ...
    model_name = cfg.get('model', {}).get('type', 'UnknownModel')
    logger = get_root_logger(log_file=log_file, log_level=cfg.get('log_level', 'INFO'), name=model_name)
    ...
    logger.info(f"data_loaders: {data_loaders}")
    # Model
    ...
    optimizer_config = OptimizerHook(grad_clip=cfg.optimizer_config.get('grad_clip', None))
    ...
    runner.register_hook(SetEpochHook(), priority='NORMAL')
    if distributed:
